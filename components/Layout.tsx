  ...
  <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 transform transition-all border border-white/20">
      <div className="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <button onClick={onClose} aria-label="Close modal" title="Close modal" className="p-2 bg-slate-100 rounded-full hover:bg-slate-200 hover:rotate-90 transition-all duration-300 text-slate-500">
          <X size={18} />
        </button>
      </div>
...
export const Layout: React.FC<LayoutProps> = ({ children, user, title }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex font-sans selection:bg-indigo-100 selection:text-indigo-700">
      ...
      {/* SIDEBAR */}
      <aside className={`
        fixed top-0 left-0 h-screen w-[280px] bg-white border-r border-slate-100 z-50
        transition-transform duration-300 ease-out flex flex-col shadow-xl
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="flex items-center justify-between mb-8 px-2">
            <div className="flex items-center gap-3">
              <img src="/src/img/learning_english_geuwat_rb_3d.png" alt="Learning English Geuwat" className="w-10 h-10 rounded-xl shadow-lg transform hover:rotate-6 transition-transform duration-300 drop-shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
              <div>
            ...
            <button onClick={() => setIsSidebarOpen(false)} aria-label="Close sidebar" title="Close sidebar" className="text-slate-400 hover:text-slate-600 transition-colors">
              <X size={24} />
        ...
        <div className="sticky top-0 z-30 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <img
              ...
              alt="Menu"
              className="w-24 h-24 cursor-pointer hover:opacity-80 transition-opacity drop-shadow-[0_0_20px_rgba(59,130,246,1)] filter brightness-125 contrast-110"
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              >
                 <Bell size={22} className="filter brightness-110" />
              ...
              {showNotification && (
                <div className="absolute top-full right-0 mt-4 w-80 sm:w-96 bg-white rounded-2xl border border-slate-100 shadow-2xl shadow-slate-200/50 z-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
                  <div className="px-5 py-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                      ...
                      <button onClick={() => setShowNotification(false)} aria-label="Close notifications" title="Close notifications" className="text-slate-400 hover:text-slate-600 flex items-center gap-1">
                        <X size={16}/>
                        <span title="Close notifications">Close</span>
                      </button>
                  </div>
        ...
        <Modal title="Your Learning Progress" onClose={closeModals}>
          {!selectedLevel ? (
            ...
            <div className="animate-in slide-in-from-right-8 duration-300">
               <button onClick={() => setSelectedLevel(null)} aria-label="Change level" className="mb-6 text-xs font-bold text-slate-400 hover:text-indigo-600 flex items-center gap-1 transition-colors">
                 ...
                 {getProgressData(selectedLevel).map((stat) => (
                   <div key={stat.subject}>
                     ...
                     <div className="w-full bg-slate-100 rounded-full h-3 overflow-hidden shadow-inner">
                       <div
                         className="progress-bar bg-gradient-to-r from-indigo-500 to-purple-500 h-full rounded-full transition-all duration-1000 ease-out shadow-lg"
                       />
                     </div>
                   </div>
                 ))}
      ...
      {activeModal === 'TUTORIAL' && (
        ...
        >
          {!viewingTutorial ? (
            ...
            <div className="animate-in slide-in-from-right-8 duration-300">
                <button onClick={() => setViewingTutorial(null)} title="Back to List" className="mb-6 text-xs font-bold text-slate-400 hover:text-slate-600 flex items-center gap-1 transition-colors">
